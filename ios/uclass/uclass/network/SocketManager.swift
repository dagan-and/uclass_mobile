...
class SocketManager: NSObject, ObservableObject {
    ...
    private func handleStompMessage(_ message: StompMessage) {
        ...
        case StompCommand.message:
            ...
            Logger.dev("STOMP message received for destination: \(destination ?? "unknown")")
        ...
        case StompCommand.receipt:
            Logger.dev("STOMP receipt: \(message.headers["receipt-id"] ?? "unknown")")
    ...
    private func sendConnectFrame() {
        let connectFrame = buildStompFrame(
            ...
            headers: [
                ...
                "heart-beat": "\(Int(clientHeartbeatInterval * 1000)),\(Int(serverHeartbeatInterval * 1000))",
                "host": String(serverUrl.dropFirst(6).split(separator: "/").first ?? ""),
                "login": String(userId)
            ],
            body: ""
        )
        ...
        Logger.dev("STOMP CONNECT frame sent with heartbeat: \(Int(clientHeartbeatInterval * 1000)),\(Int(serverHeartbeatInterval * 1000))")
    }
    ...
    private func autoSubscribeAndJoin() async {
        // 입/출 구독
        subscribe("/user/queue/dm/joined") { [weak self] message in
    ...
    private func parseAndDeliverChatMessage(_ jsonMessage: String) {
        do {
            ...
            Logger.error("ChatMessage JSON 파싱 실패: \(error.localizedDescription)")
     ...
     */
    @discardableResult
    func subscribe(_ destination: String, onMessage: @escaping (String) -> Void = { _ in }) -> String? {
     ...
     * 메시지 전송
     */
    func sendMessage(_ destination: String, message: String, headers: [String: String] = [:]) {
        guard connectionState == .connected else {
     ...
     * JSON 메시지 전송
     */
    func sendJsonMessage(_ destination: String, jsonObject: [String: Any], headers: [String: String] = [:]) {
        do {
     ...
     * STOMP 프레임 생성
     */
    private func buildStompFrame(command: String, headers: [String: String], body: String) -> String {
        var frame = command + "\n"
    ...
    private func parseHeartbeatHeader(_ heartbeatHeader: String) {
        ...
        guard parts.count == 2,
              ...
              let serverExpect = Double(parts[1]) else {
        ...
        if serverExpect > 0 && clientHeartbeatInterval > 0 {
            clientHeartbeatInterval = max(clientHeartbeatInterval, serverExpect / 1000.0)
        ...
        if serverSend > 0 && serverHeartbeatInterval > 0 {
            serverHeartbeatInterval = max(serverSend / 1000.0, serverHeartbeatInterval)
        ...
        Logger.dev("하트비트 협상 완료 - 클라이언트 송신: \(clientHeartbeatInterval)s, 서버 송신: \(serverHeartbeatInterval)s")
    }
    ...
    private func startHeartbeat() {
        ...
        if clientHeartbeatInterval > 0 {
            heartbeatTask = Task {
                while !Task.isCancelled && connectionState == .connected {
                    try? await Task.sleep(nanoseconds: UInt64(clientHeartbeatInterval * 1_000_000_000))
                    
                    if connectionState == .connected {
                        let heartbeatMessage = URLSessionWebSocketTask.Message.string("\n")
                        webSocketTask?.send(heartbeatMessage) { error in
                            if let error = error {
                                Logger.error("하트비트 전송 실패: \(error.localizedDescription)")
                            } else {
        ...
        if serverHeartbeatInterval > 0 {
            lastHeartbeatReceived = Date().timeIntervalSince1970
            let timeoutInterval = serverHeartbeatInterval * 2
            ...
            heartbeatTimeoutTask = Task {
                while !Task.isCancelled && connectionState == .connected {
                    ...
                    if timeSinceLastHeartbeat > timeoutInterval && connectionState == .connected {
                        Logger.error("서버 하트비트 타임아웃 - 연결 재시작")
                        webSocketTask?.cancel(with: .normalClosure, reason: "Heartbeat timeout".data(using: .utf8))
                        break
                    }
                }
            }
    ...
    private func scheduleReconnect() {
        ...
        guard !isReconnecting && currentReconnectAttempts < maxReconnectAttempts else {
        ...
        reconnectTask = Task {
            ...
            if shouldReconnect && isReconnecting && connectionState == .disconnected {
